...
// Busca por nome (empresa/decisor, ignorando acento/maiúsculas) OU telefone (só dígitos,
// parcial) — compartilhada entre "Todos os leads" e a busca global do header. Porta
// normalizarBusca/soDigitos/filtrarLeadsPorTexto do artefato original.
const REGEX_DIACRITICOS = /[̀-ͯ]/g;

export function normalizarBusca(s) {
  return String(s || '').normalize('NFD').replace(REGEX_DIACRITICOS, '').toLowerCase();
}

export function soDigitosBusca(s) {
  return String(s || '').replace(/\D/g, '');
}

export function filtrarLeadsPorTexto(lista, filtro) {
  if (!filtro) return lista;
  const filtroNome = normalizarBusca(filtro);
  const filtroDigitos = soDigitosBusca(filtro);
  return lista.filter((l) => {
    const nomeMatch = normalizarBusca(l.empresa).includes(filtroNome) || normalizarBusca(l.decisor).includes(filtroNome);
    const telMatch = filtroDigitos.length > 0 && soDigitosBusca(l.telefone).includes(filtroDigitos);
